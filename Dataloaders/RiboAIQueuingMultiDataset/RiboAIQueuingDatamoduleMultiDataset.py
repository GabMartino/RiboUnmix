...
class SortedLengthBatchSampler(Sampler):
    """
    Batches samples in (roughly) same-length groups to minimize padding.
    `lengths` must be aligned with `data_source` indexing (Subset-space).
    """
    def __init__(
        self,
        data_source,
        ...
        # rng state advances each epoch because __iter__ is called each epoch
...
class RiboAIQueuingDatamoduleMultiDataset(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        batch_sampler = SortedLengthBatchSampler(
            ...
            shuffle=True,
            lengths=self.train_lengths,  # FIX: correct attribute name
            seed=self.seed,
            descending=True,
        )
        return DataLoader(
