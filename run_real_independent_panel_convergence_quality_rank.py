#!/usr/bin/env python3
"""Launch the quality-ranked sensitivity of independent-panel convergence.

This is deliberately separate from the equal-reference primary experiment.
Every panel still trains through ``main_ribounmix_multidataset.py``
and its production model, loss, sampler, checkpoint, and prediction code. The
only scientific change is the fixed gamma-reference measure: global dataset
quality-rank weights replace uniform reference weights.
"""
...
DEFAULT_OUTPUT_ROOT = (
    PROJECT_ROOT / "results/real_independent_panel_convergence_quality_rank"
)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Construct balanced disjoint real-data panels and launch the "
            "quality-ranked gamma-reference sensitivity, one model per GPU."
        )
    )
    ...
    parser.add_argument(
        "--run-id",
        default=None,
        help="Outer run identifier (default: panel_convergence_qrankp1_seed<seed>).",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Frozen global dataset-quality ranking used for gamma-reference "
            "weights. By default use data.dataset_quality_ranking.path from "
            "the production config. Ranks are never recomputed per panel."
        ),
    )
    parser.add_argument(
        "--quality-rank-power",
        ...
        default=1.0,
        help=(
            "Positive exponent applied to the production rank-derived quality "
            "weights (default: 1.0). Do not tune this from the result."
        ),
    )
    ...
    reference_group.add_argument(
        ...
        type=Path,
        help=(
            "Completed equal-weight run. Abort unless the newly "
            "constructed panel assignment and every transcript split exactly "
            "match this reference design."
        ),
    )
    reference_group.add_argument(
        "--standalone-design",
        action="store_true",
        help=(
            "Construct the quality-ranked panel assignment and transcript split "
            "from scratch without requiring an equal-weight reference run."
        ),
    )
...
def _configured_quality_ranking_path(
    *, args: argparse.Namespace, base_config: Mapping[str, Any]
) -> Path:
    if args.quality_ranking_table is not None:
        return _absolute(args.quality_ranking_table)
    data_config = dict(base_config.get("data", {}) or {})
    ranking_config = dict(data_config.get("dataset_quality_ranking", {}) or {})
    raw_path = ranking_config.get("path")
    if raw_path is None or not str(raw_path).strip():
        raise ValueError(
            "The quality-ranked sensitivity requires either "
            "--quality-ranking-table or data.dataset_quality_ranking.path."
        )
    path = Path(str(raw_path)).expanduser()
    return _absolute(path if path.is_absolute() else PROJECT_ROOT / path)


def _quality_rank_reference(
    *,
    ...
    """Resolve exactly the global rank weights consumed by the model."""
    ...
    data_config = dict(base_config.get("data", {}) or {})
    ranking_config = dict(data_config.get("dataset_quality_ranking", {}) or {})
    dataset_column = str(ranking_config.get("dataset_column", "dataset"))
    rank_column = str(ranking_config.get("rank_column", "quality_rank"))
    rank_by_name, base_quality_by_name = load_dataset_quality_ranking(
        str(ranking_path),
        dataset_column=dataset_column,
        rank_column=rank_column,
    )
    missing = sorted(set(map(str, panel_datasets)) - set(rank_by_name))
    if missing:
        raise KeyError(
            "Selected panel datasets are missing from the frozen quality "
            f"ranking table {ranking_path}: {missing}."
        )
    names = list(map(str, panel_datasets))
    ranks = np.asarray([rank_by_name[name] for name in names], dtype=np.float64)
    base_weights = np.asarray(
        [base_quality_by_name[name] for name in names], dtype=np.float64
    )
    unnormalized = np.power(base_weights, float(power))
    if (
        not np.isfinite(unnormalized).all()
        or bool(np.any(unnormalized <= 0.0))
        or float(unnormalized.sum()) <= 0.0
    ):
        raise ValueError("Resolved gamma-reference rank weights are not positive.")
    pi = unnormalized / unnormalized.sum()
    ranking_sha256 = hashlib.sha256(ranking_path.read_bytes()).hexdigest()
    return {
        "dataset_names": names,
        "weighting": "quality_rank",
        "quality_rank_power": float(power),
        "ranking_table": str(ranking_path),
        "ranking_table_sha256": ranking_sha256,
        "rank_column": rank_column,
        "global_rank": {
            name: float(rank) for name, rank in zip(names, ranks, strict=True)
        },
        "base_quality_weight": {
            name: float(value)
            for name, value in zip(names, base_weights, strict=True)
        },
        "unnormalized_weight": {
            name: float(value)
            for name, value in zip(names, unnormalized, strict=True)
        },
        "pi": {
            name: float(value) for name, value in zip(names, pi, strict=True)
        },
        "pi_sum": float(pi.sum()),
        "effective_reference_dataset_count": float(1.0 / np.square(pi).sum()),
        "ranks_recomputed_within_panel": False,
    }


def _assert_reference_design_matches(
    *,
    reference_root: Path,
    assignment: pd.DataFrame,
    common_split: Mapping[str, Any] | None,
) -> None:
    reference_assignment_path = reference_root / "panel_assignment.csv"
    if not reference_assignment_path.is_file():
        raise FileNotFoundError(
            f"Reference panel assignment is missing: {reference_assignment_path}"
        )
    reference_assignment = pd.read_csv(reference_assignment_path)
    required = {"dataset_name", "panel", "source_identifier"}
    for label, frame in (
        ("new", assignment),
        ("reference", reference_assignment),
    ):
        missing = required - set(frame.columns)
        if missing:
            raise KeyError(f"{label} assignment lacks columns {sorted(missing)}.")
    columns = ["dataset_name", "panel", "source_identifier"]
    new_rows = assignment[columns].astype(str).sort_values("dataset_name").reset_index(drop=True)
    old_rows = reference_assignment[columns].astype(str).sort_values("dataset_name").reset_index(drop=True)
    if not new_rows.equals(old_rows):
        raise RuntimeError(
            "Quality-ranked sensitivity did not reproduce the equal-weight "
            "dataset/source-family panel assignment. Refusing a confounded comparison."
        )
    if common_split is None:
        return
    reference_split_path = reference_root / "common_split_manifest.json"
    if not reference_split_path.is_file():
        raise FileNotFoundError(
            f"Reference common split is missing: {reference_split_path}"
        )
    reference_split = json.loads(reference_split_path.read_text(encoding="utf-8"))
    for key in ("common_validation_ids", "common_test_ids"):
        if list(map(str, common_split[key])) != list(map(str, reference_split[key])):
            raise RuntimeError(
                f"Quality-ranked sensitivity {key} differs from the equal-weight run."
            )
    new_train = common_split["panel_train_eligible_ids"]
    old_train = reference_split["panel_train_eligible_ids"]
    if set(new_train) != set(old_train):
        raise RuntimeError("Reference and sensitivity split panel names differ.")
    for panel in new_train:
        if list(map(str, new_train[panel])) != list(map(str, old_train[panel])):
            raise RuntimeError(
                f"Training transcript IDs differ from the equal-weight run for {panel}."
            )
...
def _flatten_config(value: Any, *, prefix: str = "") -> dict[str, Any]:
    ...
    for key, child in value.items():
        ...
        flattened.update(_flatten_config(child, prefix=path))
    return flattened


def _assert_only_gamma_reference_changed(
    *,
    reference_root: Path,
    panel_name: str,
    resolved_config: Mapping[str, Any],
) -> None:
    """Reject accidental hyperparameter drift from the paired equal run."""
    reference_path = reference_root / panel_name / "resolved_config.yaml"
    if not reference_path.is_file():
        raise FileNotFoundError(
            f"Reference resolved config is missing: {reference_path}"
        )
    reference = _load_yaml(reference_path)
    old = _flatten_config(reference)
    new = _flatten_config(resolved_config)
    allowed_exact = {
        "name",
        "split.external_manifest",
        "data.reliability_reference_manifest",
        "data.dataset_quality_ranking.path",
        "model.gamma_centering.reference.weighting",
        "model.gamma_centering.reference.quality_rank_power",
    }
    mismatches: dict[str, dict[str, Any]] = {}
    for key in sorted(set(old) | set(new)):
        if key in allowed_exact or key.startswith(("paths.", "orchestrator.")):
            continue
        old_value = old.get(key)
        new_value = new.get(key)
        if key == "trainer.detect_anomaly" and old_value in (None, False) and new_value in (None, False):
            continue
        if old_value != new_value:
            mismatches[key] = {"equal_weight": old_value, "quality_rank": new_value}
    if mismatches:
        examples = dict(list(mismatches.items())[:20])
        raise RuntimeError(
            "Training configuration drifted beyond the requested gamma-reference "
            f"weighting change for {panel_name}: {examples}"
        )
    if old.get("model.gamma_centering.reference.weighting") != "equal":
        raise RuntimeError(
            f"Reference config for {panel_name} is not the expected equal-weight run."
        )
    if new.get("model.gamma_centering.reference.weighting") != "quality_rank":
        raise RuntimeError(
            f"Sensitivity config for {panel_name} is not quality-ranked."
        )
...
def _resolved_panel_config(
    *,
    ...
    overrides = {
        ...
        "model.gamma_centering.reference.quality_rank_power": float(
            args.quality_rank_power
        ),
        "data.dataset_quality_ranking.path": str(args.quality_ranking_table),
    ...
    resolved["orchestrator"] = {
        "experiment": "independent_dataset_panel_convergence_quality_rank_sensitivity",
        "panel": panel_name,
        "run_name": run_name,
        "created_at_utc": utc_timestamp(),
        "controlled_overrides": overrides,
        "reliability_weight_mode": str(args.reliability_weight_mode),
        "quality_ranking_table": str(args.quality_ranking_table),
        "quality_rank_power": float(args.quality_rank_power),
    }
    return resolved
...
def _training_command(
    *,
    ...
    command = [
        ...
        (
            "model.gamma_centering.reference.quality_rank_power="
            f"{float(args.quality_rank_power)}"
        ),
        f"data.dataset_quality_ranking.path={args.quality_ranking_table}",
...
def _contract_rows(
    *,
    ...
    quality_rank_pi = True
    ranking_hashes: set[str] = set()
    for panel, panel_names in panels.items():
        reference = run_manifests[panel]["fixed_gamma_reference"]
        pi_by_name = reference.get("pi", {})
        raw_by_name = reference.get("unnormalized_weight", {})
        names_match = set(pi_by_name) == set(panel_names) == set(raw_by_name)
        if names_match:
            pi_values = np.asarray(
                [float(pi_by_name[name]) for name in panel_names], dtype=np.float64
            )
            raw_values = np.asarray(
                [float(raw_by_name[name]) for name in panel_names], dtype=np.float64
            )
            expected_pi = raw_values / raw_values.sum()
        else:
            pi_values = np.asarray([], dtype=np.float64)
            expected_pi = np.asarray([], dtype=np.float64)
        valid = (
            reference.get("weighting") == "quality_rank"
            and float(reference.get("quality_rank_power", 0.0)) > 0.0
            and names_match
            and bool(np.isfinite(pi_values).all())
            and bool(np.all(pi_values > 0.0))
            and bool(np.allclose(pi_values, expected_pi, rtol=1.0e-12, atol=1.0e-12))
            and math.isclose(float(pi_values.sum()), 1.0, abs_tol=1.0e-12)
            and reference.get("ranks_recomputed_within_panel") is False
        )
        quality_rank_pi = quality_rank_pi and valid
        ranking_hashes.add(str(reference.get("ranking_table_sha256", "")))
    one_frozen_ranking = len(ranking_hashes) == 1 and "" not in ranking_hashes
    paired_design_roots = {
        str(manifest.get("paired_equal_weight_reference_root", ""))
        for manifest in run_manifests.values()
    }
    paired_equal_design_recorded = (
        len(paired_design_roots) == 1 and "" not in paired_design_roots
    )
    paired_training_config_audited = all(
        str(manifest.get("paired_training_configuration_audit", "")).startswith(
            "PASS:"
        )
        for manifest in run_manifests.values()
    )
    ...
    ranked_centering = all(
        config["model"]["gamma_centering"]["reference"]["weighting"]
        == "quality_rank"
        and float(
            config["model"]["gamma_centering"]["reference"][
                "quality_rank_power"
            ]
        )
        > 0.0
    ...
    design_contract = (
        [
            (
                "standalone panel design is explicitly recorded",
                all(
                    manifest.get("design_mode") == "standalone"
                    for manifest in run_manifests.values()
                ),
                "required",
            )
        ]
        if standalone_design
        else [
            (
                "exact paired equal-weight design is recorded",
                paired_equal_design_recorded,
                "required",
            ),
            (
                "all non-gamma training settings match equal-weight run",
                paired_training_config_audited,
                "required",
            ),
        ]
    )
    rows = [
        ...
        (
            "gamma pi follows the frozen global quality ranking",
            quality_rank_pi and ranked_centering,
            "required",
        ),
        ("one frozen rank table is used by every panel", one_frozen_ranking, "required"),
        *design_contract,
    ...
    for panel_index, (panel_name, panel_datasets) in enumerate(
        ...
        if args.reliability_weight_mode == "train-only-snr":
            ...
            reliability_manifest = fit_panel_reliability_manifest(
                experiment_name=(
                    "independent_dataset_panel_convergence_quality_rank_sensitivity"
                ),
        ...
        run_name = f"real_panel_qrank_convergence_panel{panel_index:02d}"
        ...
        if args.reference_design_root is not None:
            _assert_only_gamma_reference_changed(
                reference_root=args.reference_design_root,
                panel_name=panel_name,
        ...
        ranked_reference = _quality_rank_reference(
            panel_datasets=panel_datasets,
            ranking_path=args.quality_ranking_table,
        ...
        write_json(
            panel_directory / "run_manifest.json",
            {
                ...
                "scientific_role": (
                    "standalone quality-ranked independent-panel experiment"
                    if args.standalone_design
                    else "quality-ranked gamma-reference sensitivity"
                ),
                "design_mode": (
                    "standalone" if args.standalone_design else "paired_equal_weight"
                ),
                "paired_training_configuration_audit": (
                    "NOT_APPLICABLE: standalone design"
                    if args.standalone_design
                    else "PASS: only gamma-reference weighting/power and run/output "
                         "identity fields differ from equal-weight reference"
                ),
                "paired_equal_weight_reference_root": (
                    None if args.reference_design_root is None
                    else str(args.reference_design_root)
                ),
                "panel_name": panel_name,
                ...
                "fixed_gamma_reference": ranked_reference,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.reference_design_root is not None:
        args.reference_design_root = _absolute(args.reference_design_root)
    if args.reference_design_root is not None and not args.reference_design_root.is_dir():
        raise FileNotFoundError(
            f"Equal-weight reference design root does not exist: "
            f"{args.reference_design_root}"
        )
    ...
    if not math.isfinite(args.quality_rank_power) or args.quality_rank_power <= 0.0:
        raise ValueError("--quality-rank-power must be finite and strictly positive.")
    gpus = _parse_gpus(args.gpus)
    base_config = _load_yaml(args.config)
    args.quality_ranking_table = _configured_quality_ranking_path(
        args=args, base_config=base_config
    )
    if not args.quality_ranking_table.is_file():
        raise FileNotFoundError(
            f"Dataset-quality ranking table does not exist: "
            f"{args.quality_ranking_table}"
        )
    ...
    power_tag = format(float(args.quality_rank_power), "g").replace(".", "p")
    run_id = args.run_id or f"panel_convergence_qrankp{power_tag}_seed{args.seed}"
    ...
    if args.reference_design_root is not None:
        _assert_reference_design_matches(
            reference_root=args.reference_design_root,
    ...
    panel_manifest = {
        ...
        "design_mode": "standalone" if args.standalone_design else "paired_equal_weight",
        "paired_equal_weight_reference_root": (
            None if args.reference_design_root is None else str(args.reference_design_root)
        ),
        ...
        "reliability_weight_mode": str(args.reliability_weight_mode),
        "gamma_reference_strategy": {
            "weighting": "quality_rank",
            "quality_rank_power": float(args.quality_rank_power),
            "ranking_table": str(args.quality_ranking_table),
            "ranking_table_sha256": hashlib.sha256(
                args.quality_ranking_table.read_bytes()
            ).hexdigest(),
            "rank_scope": "frozen_global_table; never recomputed per panel",
            "scientific_role": (
                "standalone quality-ranked experiment"
                if args.standalone_design else "secondary sensitivity analysis"
            ),
            "primary_comparator": (
                None if args.reference_design_root is None
                else str(args.reference_design_root)
    ...
    contract = _contract_rows(
        ...
        standalone_design=args.standalone_design,
    )
