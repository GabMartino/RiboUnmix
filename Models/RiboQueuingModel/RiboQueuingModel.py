...
#         )
#         ## TODO: try single \pi for the whole transcript
...
#
#         # FIX: Rename to avoid shadowing the [B, E] argument
...
#
#         ff_input = torch.cat(
#             [out_for_scale, dataset_emb_expanded], # Use the safely renamed variable
#             dim=-1,
#         )
#
...
#             Intrinsic biology
#         '''
#         # FIX 1: Remove .squeeze() to protect against Batch=1 crashes. Shape is strictly [B, E].
...
#
#         # FIX 2: Create a local expanded variable. Leave dataset_embeddings as [B, E]
#         dataset_emb_expanded = dataset_embeddings.unsqueeze(1).expand(B, T, -1)
#         pi_ff_input = torch.cat([out_detached, dataset_emb_expanded], dim=-1)
#
#         pi = self.ff_pi(pi_ff_input).squeeze(-1) * self.pi_max
#         pi = pi * mask_f
#
#         # FIX 3: Pass the original [B, E] tensor, because the function expands it internally
