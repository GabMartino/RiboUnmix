...
class RiboUnmixModel(nn.Module):
    """
    Interpretable shared-profile and dataset-bias decomposition:
    ...
    where
        ...
        rho[t,i]   = L_bio / (1 + L_bio), a bounded diagnostic transform.
    ...
    def __init__(
        ...
        self.mean_correction = str(model_configs.get("mean_correction", "learned"))
        if self.mean_correction not in {"learned", "unity"}:
            raise ValueError("model.mean_correction must be 'learned' or 'unity'.")
        ...
        self.biological_model = SharedProfileModel(config_params=biological_params)
    ...
    def _configure_gamma_centering(
        ...
        *,
        ...
        if weighting not in {"equal", "quality_rank", "explicit"}:
            raise ValueError(
                "gamma_centering weighting must be 'equal', 'quality_rank', or 'explicit'."
            )
        ...
        if weighting == "explicit":
            explicit = dict(reference_cfg.get("explicit_weights", {}) or {})
            if mode != "fixed_reference" or set(explicit) != set(names):
                raise ValueError("Explicit weights must cover exactly the fixed reference dataset identities.")
            final_weights = torch.tensor([float(explicit[name]) for name in names], dtype=torch.float64)
            if not torch.isfinite(final_weights).all() or not (final_weights > 0).all():
                raise ValueError("Explicit reference weights must be finite and strictly positive.")
            if quality_rank_power != 1.0:
                raise ValueError("Explicit reference weights are used as supplied, with power one.")
        elif weighting == "quality_rank" and quality:
    ...
    def set_extra_state(self, state: dict | None) -> None:
        if not state:
            return
        self.mean_correction = str(state.get("mean_correction", "learned"))
        if self.mean_correction not in {"learned", "unity"}:
            raise ValueError("Unknown checkpoint mean-correction intervention.")
    ...
    def forward(
        ...
        if self.mean_correction == "unity":
            # Exact mean ablation, not a change of centering policy. Keep the
            # existing alpha head and its detached context unchanged. Consequently
            # the context encoder receives no mean/dispersion gradient in this arm.
            log_gamma = torch.zeros_like(log_gamma)
