...
#
#
#     def __init__(self, censor_threshold: float = 0.5,
#                  eps = 1e-6):
#         super().__init__()
#
#         if censor_threshold <= 0:
#             raise ValueError("censor_threshold must be > 0 (LogNormal uses log(delta)).")
#
#         self.eps = eps
#         self.censor_threshold = censor_threshold
#
...
#         nll = torch.zeros_like(y)
#
#         # ---------- low/censored: -log(pi + (1-pi)*Phi(z_delta)) ----------
#         if low.any():
#             z = (log_delta - mu_stat[low]) / sigma[low]
#             logcdf = self._log_ndtr(z)
#             log_pi = torch.log(pi[low])
...
#         mask_f = m.to(dtype=y.dtype)
#         L = mask_f.sum(dim=1).clamp_min(1.0)
#         loss_nll = (nll * mask_f).sum(dim=1).div(L).mean()
#
#
#         return loss_nll
#
