...
def make_gamma_centering_run_tag(cfg: DictConfig) -> str:
    if str(cfg_get(cfg, "model.mean_correction", "learned")) == "unity":
        return "GammaMeanUnity"
    mode = str(
        cfg_get(
            cfg,
    ...
    if weighting == "explicit":
        return f"GammaCtr{mode_tag}Explicit"
...
def shared_logger_version_from_environment() -> str | None:
    ...
    torchrun uses ``RIBOUNMIX_LOGGER_VERSION`` for the same purpose.
    """
    ...
    explicit_version = os.environ.get("RIBOUNMIX_LOGGER_VERSION")
    if explicit_version is None:
        # Compatibility for frozen launch scripts created before the rename.
...
def make_datamodule(
    *,
    ...
    return RiboUnmixMultiDatasetDataModule(
...
@hydra.main(
    ...
    config_name="config_ribounmix_multidataset",
)
def main(cfg: DictConfig) -> None:
    ...
    if external_split_manifest is not None:
        ...
        print(f"panel validation IDs:     {len(validation_fold):,}")
        ...
        test_fold = []
    expected_split_manifest = cfg_get(cfg, "split.expected_manifest", None)
    if expected_split_manifest:
        assert_expected_transcript_split(
        ...
        print("Historical train/validation identities and ordering: matched.")
    ...
    campaign_task_contract = cfg_get(cfg, 'orchestrator.task_contract_sha256', None)
    if campaign_task_contract is not None:
        lit_model.hparams['campaign_task_contract_sha256'] = campaign_task_contract

    # The split provider controls transcript membership. External manifests
    # may use panel-specific folds; common observation coverage is guaranteed
    # only when their design explicitly enforces it. The dataloader intersects
    # each fold with usable rows independently for each selected dataset.
